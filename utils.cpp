...
int create_sprite_buffer_screen() {
  screen_sprite_buffer = create_memory_bitmap(
      32, 32); // create ALLEGRO_BITMAP pour le petit buffer
  return !screen_sprite_buffer;
}

ALLEGRO_BITMAP *sprite_buffer_screen() { return screen_sprite_buffer; }

void clean_sprite_buffer_screen() {
  if (screen_sprite_buffer)
    al_destroy_bitmap(screen_sprite_buffer);
}
